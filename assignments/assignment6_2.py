...
income_returns, price_returns, total_returns, new_portfolio, portfolio_values, transaction_costs, \
               weekly_fees, dividends_collection = Helpers.load_vars_from_pickle('pkls/a61.pkl')

new_portfolio.reset_dividends()

# For the weeks from 1 May to 8 May, and from 8 May to 15 May, check your portfolio for any splits and
# adjust holdings accordingly. Check for any dividends and add to the liquidity reserve accordingly.
# Calculate the management fee and deduct from the liquidity reserve. Trading is optional. Use the
# writeHoldings function to write holdings text files for 2020-05-08 and for 2020-05-15.
# Value the portfolio as of 8 May and 15 May and calculate the portfolio income return, price return, and
# total rate of return for the periods from 2020-05-01 to 2020-05-08 and from 2020-05-08 to 2020-05-15.
# Use the writeAccountSummary function to write an account summary text file for the eight periods through
# 2020-05-15. Submit the holdings and account summary.
...
Helpers.myprint([f'The cash before discounting the fee is: {round(portfolio_cash_before_fee, 2)}',
                 f'The weekly fee on: {Helpers.assignment6_1_end_date} is: {round(weekly_fee, 2)}, '
...
IoHelpers.write_holdings('yassers', new_portfolio, Helpers.assignment6_2_end_date)

portfolio_value_by_the_end_of_current_week = new_portfolio.get_value(Helpers.assignment6_2_end_date)
...
income_return, price_return, total_return = \
    Helpers.get_returns(new_portfolio, portfolio_values[len(portfolio_values) - 2], Helpers.assignment6_2_end_date)
...
IoHelpers.write_account_summary(
    ...
    dates=[Helpers.start_date,
           ...
           Helpers.assignment6_1_end_date,
           Helpers.assignment6_2_end_date],
    deposits=[Helpers.that_initial_balance, 0, 100_000, 0, 0, 0, 0, 0],
...
Helpers.save_to_pickle(collection, 'pkls/a62.pkl')
